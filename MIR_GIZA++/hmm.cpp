...
//@ASHISH VASWANI
int hmm::em_with_tricks_e_step(int iterationNumber,
    bool blah1,
    Dictionary *blah2,
    bool blah3,
    string model_prefix,
    vector<Array<double> > &posteriors)
{
  ...
  afile = Prefix + ".a" + model_prefix + "." + shortModelName + "." + number ;
  afileh = Prefix + ".h" + model_prefix + "." + shortModelName + "." + number ;
...
minErrors=errorsAL();
  ...
  if (testPerp && testHandler){
    vector<Array<double> > test_posteriors;
    em_loop(*testPerp,
        *testHandler,
        dump_files,
        test_alignfile.c_str(),
        *testViterbiPerp, 
        true,
        it==1,
        it,
...
//    tTable.printCountTable(tfile.c_str(),Elist.getVocabList(),Flist.getVocabList(),1);
  ...
  probs=counts;
    //THE T-TABLE MUST NOT BE NORMALIZED BECAUSE WE WILL BE 
    //CALLING PGD 
    ...
    */
    it_fn = time(NULL) ;
    cout << "\n" << modelName << " Iteration: " << iterationNumber<< " took: " <<
      difftime(it_fn, it_st) << " seconds\n";
  //} // end of iterations 
  fn = time(NULL) ;
  //cout << endl << "Entire " << modelName << " Training took: " << difftime(fn, st) << " seconds\n";
  cout << "Model hmm Iteration: " << it<< " took: " << difftime(fn, st) << " seconds\n";
...
void hmm::printTableAndReport(int it,
    string model_prefix) {
  ...
  dump_files = (ModelH_Dump_Freq != 0) && ((it % ModelH_Dump_Freq) == 0) && !NODUMPS;
  number = "";
  int n = it;
  do{
    number.insert((size_t)0, 1, (char)(n % 10 + '0'));
...
void hmm::em_loop(Perplexity& perp, sentenceHandler& sHandler1, 
		  bool dump_alignment, const char* alignfile, Perplexity& viterbi_perp, 
		     bool test,bool doInit,int, vector<Array<double> >&posteriors)
{
  WordIndex i, j, l, m ;
  ...
  while(sHandler1.getNextSentence(sent)){
    ...
    double trainProb;
      ...
      //Now that we have the gamma, we add it to the posteriors
      posteriors.push_back(gamma);
    if( !test )
      {
