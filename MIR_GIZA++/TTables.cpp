    ...
    if( lexmat[i] )
	  {
        ...
        rowExpCntsVec.push_back((*lexmat[i])[j].second.count);
        (*lexmat[i])[j].second.count = 0;
      }
	  }
    expCntsVec->push_back(rowExpCntsVec);
    rowwiseExpCntsSum->push_back(c);
  }
}
    ...
    if( lexmat[i] )
	  {
	    unsigned int lSize=lexmat[i]->size();
	    for(unsigned int j=0;j<lSize;++j) {
        rowProbsVec.push_back((*lexmat[i])[j].second.prob);
      }
	  }
...
template <class COUNT, class PROB>
void tmodel<COUNT,PROB>::setProbs(const vector<vector<PROB> >&probsVec) {
  for(unsigned int i=0;i<lexmat.size();++i)
  {
    if( lexmat[i] )
	  {
	    unsigned int lSize=lexmat[i]->size();
	    for(unsigned int j=0;j<lSize;++j) {
        (*lexmat[i])[j].second.prob = probsVec[i][j];
      }
	  }
  }
}
...
template <class COUNT, class PROB>
void tmodel<COUNT, PROB>::normalizeTableDontZero(const vcbList&, const vcbList&, int)
{
  for(unsigned int i=0;i<lexmat.size();++i)
    {
      double c=0.0;
      if( lexmat[i] )
	{
	  ...
	  for(unsigned int j=0;j<lSize;++j)
		...
		(*lexmat[i])[j].second.prob=(*lexmat[i])[j].second.count/c;
		  //Does not zero out the expected count because it is needed for the PGD m-step with the regularizer
	      //(*lexmat[i])[j].second.count=0;
	    }
	}
    }
