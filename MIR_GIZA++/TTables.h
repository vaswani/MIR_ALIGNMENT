...
public:
  ...
  //A version of normalize table where the expected counts are not zeroed out
  void normalizeTableDontZero(const vcbList&engl, const vcbList&french, int iter=2);
  void getCounts(vector<vector<float> >* expCntsVec,vector<float> *rowwiseExpCntsSum) const  ;
  ...
  const vector< vector<pair<unsigned int,CPPair> >* > &getLexmat() const {
    return (lexmat); }
