...
//@ASHISH VASWANI. VITERBI EM WITH COUPLING SINGLE STEP
int model3::viterbiModelCouplingSingleStep(int it,
    d4model &d4m,
    d5model &d5m,
    char fromModel,
    char toModel,
  ...
  */
  ...
  //    final=1;
    ...
    if(fromModel==toModel)
      ...
      modelName=string("T")+fromModel+"To"+toModel;
    it_st = time(NULL);
    cout <<"\n---------------------\n"<<modelName<<": Iteration of direction "<<model_prefix <<":"<< it<<'\n';
    if (Log) 
      logmsg <<"\n---------------------\n"<<modelName<<": Iteration of direction "<<model_prefix <<":"<< it<<'\n';
    dump_files = (final || ((Model3_Dump_Freq != 0) && ((it % Model3_Dump_Freq) == 0))) && !NODUMPS ;
    string d4file2;
	...
	number.insert((size_t) 0, 1, (char)(n % 10 + '0'));
      ...
      if( final )
	      number="final";
      tfile = Prefix + ".t3." + model_prefix + "." + number ;
      tfile_actual = Prefix + ".actual.t3." + model_prefix + "." + number ;
      afile = Prefix + ".a3." + model_prefix + "." + number ;
      nfile = Prefix + ".n3." + model_prefix + "." + number ;
      nfile_actual = Prefix + ".actual.n3." + model_prefix + "." + number ;
      dfile = Prefix + ".d3." + model_prefix + "." + number ;
      d4file = Prefix + ".d4." + model_prefix + "." + number ;
      d4file2 = Prefix + ".D4." + model_prefix + "." + number ;
      d5file = Prefix + ".d5." + model_prefix + "." + number ;
      alignfile = Prefix + ".A3." + model_prefix + "." + number ;
      test_alignfile = Prefix + ".tst.A3." + model_prefix + "." + number ;
      p0file = Prefix + ".p0_3." + model_prefix + "." + number ;
    }
    // clear count tables 
    //    tCountTable.clear();
    dCountTable.clear();
    ...
    d4m.clear();
    p0_count = p1_count = 0 ;
    //cout<<"About to start tricy ibm training "<<endl;

#ifdef TRICKY_IBM3_TRAINING
    
#define TRAIN_ARGS_COUPLE perp,      trainViterbiPerp, sHandler1,    dump_files, alignfile.c_str(),     true,  modelName,final
#define TEST_ARGS_COUPLE  *testPerp, *testViterbiPerp, *testHandler, dump_files, test_alignfile.c_str(),false, modelName,final

  //cout<<"we are in tricy ibm model training"<<endl;
    switch( toModel )
      {
      case '3':
	switch(fromModel )
	  {
	  case 'H':
      //cout<<"we are in case H"<<endl;
	    viterbi_loop_with_tricks  <transpair_modelhmm,const hmm>(TRAIN_ARGS_COUPLE,h,(void*)0);
      //cout<<"we ran viterbi loop with tricks"<<endl;
	    if (testPerp && testHandler)
	      viterbi_loop_with_tricks<transpair_modelhmm,const hmm>(TEST_ARGS_COUPLE, h,(void*)0);
	    break;
	  case '3':
      //cout<<"we are in case 3"<<endl;
	    viterbi_loop_with_tricks<transpair_model3>( TRAIN_ARGS_COUPLE, (void*)0,(void*)0);
	    if (testPerp && testHandler)
	      viterbi_loop_with_tricks<transpair_model3>( TEST_ARGS_COUPLE, (void*)0,(void*)0);
	    break;
	  default: {
               //cout<<"we are in default for model 3"<<endl;
               abort();
             }
	  }
	break;
      case '4':
	{
	  switch(fromModel)
	    {
	    case 'H':
	      viterbi_loop_with_tricks  <transpair_modelhmm,const hmm,d4model>(TRAIN_ARGS_COUPLE,h,&d4m);
	      if (testPerp && testHandler)
		viterbi_loop_with_tricks<transpair_modelhmm,const hmm,d4model>(TEST_ARGS_COUPLE, h,&d4m);
	      break;
	    case '3':
	      viterbi_loop_with_tricks<transpair_model3, void,d4model>(TRAIN_ARGS_COUPLE, (void*)0,&d4m);
	      if (testPerp && testHandler)
		viterbi_loop_with_tricks<transpair_model3, void,d4model>( TEST_ARGS_COUPLE , (void*)0,&d4m);	
	      break;
	    case '4':
	      viterbi_loop_with_tricks<transpair_model4, d4model,d4model>(TRAIN_ARGS_COUPLE , &d4m,&d4m);
	      if (testPerp && testHandler)
		viterbi_loop_with_tricks<transpair_model4, d4model,d4model>( TEST_ARGS_COUPLE, &d4m,&d4m);
	      break;
	    default: abort();
	    }
	  d4m.normalizeTable();
      ...
      case '5':
	{
	  switch(fromModel)
	    {
	    case 'H':
	      viterbi_loop_with_tricks  <transpair_modelhmm,const hmm,d5model>(TRAIN_ARGS_COUPLE,h,&d5m);
	      if (testPerp && testHandler)
		viterbi_loop_with_tricks<transpair_modelhmm,const hmm,d5model>(TEST_ARGS_COUPLE, h,&d5m);
	      break;
	    case '3':
	      viterbi_loop_with_tricks<transpair_model3, void,d5model>(TRAIN_ARGS_COUPLE, (void*)0,&d5m);
	      if (testPerp && testHandler)
		viterbi_loop_with_tricks<transpair_model3, void,d5model>( TEST_ARGS_COUPLE , (void*)0,&d5m);	
	      break;
	    case '4':
	      viterbi_loop_with_tricks<transpair_model4, d4model,d5model>(TRAIN_ARGS_COUPLE, &d4m,&d5m);
	      if (testPerp && testHandler)
		viterbi_loop_with_tricks<transpair_model4, d4model,d5model>( TEST_ARGS_COUPLE, &d4m,&d5m);	
	      break;
	    case '5':
	      viterbi_loop_with_tricks<transpair_model5, d5model, d5model>(TRAIN_ARGS_COUPLE, &d5m,&d5m);
	      if (testPerp && testHandler)
		viterbi_loop_with_tricks<transpair_model5, d5model, d5model>( TEST_ARGS_COUPLE, &d5m,&d5m);	
	      break;
	    default: abort();
	    }
	  d5m.d4m.normalizeTable();
      ...
      default:
      {
        //cerr<<"We entered default mode"<<endl;
        abort();
      }
      }
    //cout<<"We just got out of the switch case statment"<<endl;
...
#endif		 
    //cout<<"Finished tricky IBM training "<<endl;
    if( errorsAL()<minErrors )
      {
	minErrors=errorsAL();
        minIter=it;
    ...
    //We will normalize the table in pgd land
    tTable.normalizeTableDontZero(Elist, Flist);
  ...
  */
  ...
  if( noIterationsModel5 )
    minIter-=noIterationsModel4;
  */
  cout<<"returning from tricky ibm 3 training "<<endl;
  return (0);
}

void model3::printTableAndReport(int it,
    string model_prefix,
    char fromModel,
    char toModel,
    bool final) {
  ...
  string tfile = Prefix + ".t3." + model_prefix + "." + number ;
  //string modelName="Hmm",shortModelName="hmm";
    ...
    if (testPerp && testHandler)
      cout << modelName << ":("<<it<<" TEST CROSS-ENTROPY for model " <<model_prefix<<" is: "<< (*testPerp).cross_entropy()
	   << " PERPLEXITY " << (*testPerp).perplexity() << " sum: " << (*testPerp).getSum()<<
	" wc: " << (*testPerp).word_count() << '\n';
    cout << modelName << ": ("<<it<<") TRAIN VITERBI CROSS-ENTROPY for model "<<model_prefix<<" is: " << trainViterbiPerp.cross_entropy()
	 << " PERPLEXITY " << trainViterbiPerp.perplexity() << '\n';
    if (testPerp && testHandler)
      cout << modelName << ":  ("<<it<<")TEST VITERBI CROSS-ENTROPY for model " <<model_prefix<<" is: "<< (*testViterbiPerp).cross_entropy()
	   << " PERPLEXITY " << (*testViterbiPerp).perplexity() << " Sum: " << (*testViterbiPerp).getSum() <<
	" wc: " << (*testViterbiPerp).word_count() << '\n';
    if (dump_files)
      {
